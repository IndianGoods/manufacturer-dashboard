      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={formData.maxUses}
              onChange={(e) => handleInputChange("maxUses", e.target.checked)}
            />{" "}
          ...
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={formData.maxUsesPerCustomer}
              onChange={(e) =>
                handleInputChange("maxUsesPerCustomer", e.target.checked)
              }
            />{" "}
      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={formData.combinations.product}
              onChange={(e) =>
                handleInputChange("product", e.target.checked, "combinations")
              }
            />{" "}
            Product discounts
          </label>
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={formData.combinations.order}
              onChange={(e) =>
                handleInputChange("order", e.target.checked, "combinations")
              }
            />{" "}
            Order discounts
          </label>
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={formData.combinations.shipping}
              onChange={(e) =>
                handleInputChange("shipping", e.target.checked, "combinations")
              }
            />{" "}
            Shipping discounts
          </label>
...
const DiscountSummaryCard = ({ formData }) => (
  <Card>
    ...
    <Card.Content>
      ...
      <div className="text-xs text-gray-500 mb-2">
        Amount off: {formData.discountValue || "-"}{" "}
        {formData.discountType === "percentage" ? "%" : "₹"}
      </div>
      <div className="text-xs text-gray-500 mb-2">
        Target: {formData.discountTarget === "product" ? "Product" : "Order"}
      </div>
      ...
      <ul className="text-xs text-gray-500 list-disc pl-4">
        ...
        <li>
          {formData.minRequirement === "none"
            ...
            ? `Min amount: ₹${formData.minAmount}`
            : `Min quantity: ${formData.minQuantity}`}
        </li>
        <li>{formData.maxUses ? "Usage limit enabled" : "No usage limits"}</li>
        <li>
          {formData.combinations.product ||
          formData.combinations.order ||
          formData.combinations.shipping
  ...
  <Card>
    ...
    <Card.Content>
      <label className="flex items-center gap-2">
        <input
          type="checkbox"
          checked={formData.channelAccess}
          onChange={(e) => handleInputChange("channelAccess", e.target.checked)}
        />{" "}
...
const EditDiscount = () => {
  ...
  const discount = useSelector((state) =>
    state.discounts.items.find((d) => String(d.id) === String(id))
  );
    ...
    if (discount) {
      ...
      setFormData({
        ...
        combinations: {
          ...
          shipping: discount.combinations?.shipping ?? false,
        },
      });
    }
  }, [discount]);
  ...
  const appliesToOptions = {
    ...
    order: [{ value: "all", label: "Entire order" }],
  };
  ...
  const handleSave = () => {
    dispatch(updateDiscount({ ...formData, id }));
